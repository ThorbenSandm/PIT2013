	...
	while (getline (Schaltnetzdatei, zeile)) {
		if (zeile.substr(0,5)=="INPUT") {
			...
			do {
				...
				signale[name-1].setSignalTyp(Signal::eingang); //Speichern der Information in das richtige Objekt
			} 
			...
			while (poskom=a.find(",",0)!=-1) {
				...
				a=a.erase(0,5); //Das extrahierte Signal wird aus dem String gelöscht
				int name =atol(b.c_str()); //Umwandeln der Namenszahl in eine Integervariable
				signale[name-1].setSignalTyp(Signal::ausgang); //Speichern der Information in das richtige Objekt
			} 
			string c=a.substr(1,3);
			int name =atol(c.c_str());
			signale[name-1].setSignalTyp(Signal::ausgang);
			...
			while (poskom=a.find(",",0)!=-1){
				...
				a=a.erase(0,5); //Das extrahierte Signal wird aus dem String gelöscht
				int name =atol(b.c_str()); //Umwandeln der Namenszahl in eine Integervariable
				signale[name-1].setSignalTyp(Signal::intern); //Speichern der Information in das richtige Objekt
			} 
		
			string c=a.substr(1,3);
			int name =atol(c.c_str());
			signale[name-1].setSignalTyp(Signal::intern);
			...
			signale[eingangsSignal1-1].zielHinzufuegen(gattername,signale[eingangsSignal1-1].anzahlZiele); //Hinzufügen des Ziels in den Array des Objektes, an die erste noch nicht besetzte Stelle
			
			switch (signale[eingangsSignal1-1].anzahlZiele) {   //Die AnzahlZiele des Signals wird um 1 erhöht
			...
			if (poskomma2!=-1) { //Überprüfen, ob 2. Komma im String enthalten
				if(zeile.at(poskomma1+1)!='c') { //Prüfen, ob 2. Eingang das Taktsignal ist
					...
					signale[eingangsSignal2-1].zielHinzufuegen(gattername,signale[eingangsSignal2-1].anzahlZiele); //Hinzufügen des Ziels in den Array des Objektes, an die erste noch nicht besetzte Stelle

					switch (signale[eingangsSignal2-1].anzahlZiele) {   //Die AnzahlZiele des Signals wird um 1 erhöht
...
void SignalListeErzeuger::AusgabeSignalliste(short anzahlSignale) {
	...
	for (int i=0; i<anzahlSignale; i++) {
		if (i+1<10) {
			...
			Signal::signalTypen SiTyp=signale[i].getSignalTyp();
			switch (SiTyp) {
